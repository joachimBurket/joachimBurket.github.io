...
* **PKCS#12 (PFX) conversions:**
  ...
  The reverve conversion needs two steps:
  
  1. Convert the pfx in a single pem file:

    ```bash
    openssl pkcs12 -in test.p12 -out test.pem -nodes
    ```

  2. Manually split the key, certificate and intermediate certificate by opening it in an editor.
